...
- `research_agent.contracts.agronomy.synthesis` — `NormalizedConcept`, `CrossCropPattern`, `OntologyNode`, `OntologyEdge`, `PlatformPrimitive`, `SynthesisOutput` (**schema_version `1.1`**: deterministic pattern/primitive/node ids, optional `ontology_edges`, `validation_warnings`). Cross-crop synthesis over dossier + optional questionnaire JSON; claim graphs are not used as inputs.
...
- `research-agent-synthesize` → `research_agent.cli.synthesize:main` — **core install** (Pydantic + contracts only; no retrieval imports)
  - **`--inputs-manifest PATH`** or **`--input-dir DIR`** (loads `DIR/manifest.json`) — validated **`SynthesisManifest`** (`research_agent.synthesis.manifest`): `runs[]` (or legacy **`inputs[]`**) with `run_id`, **`dossier`** (path **relative** to the manifest directory; absolute paths and `..` traversal are rejected), optional **`questionnaire`**, optional **`questionnaire_spec`** (relative path to `QuestionnaireSpec` JSON for `question_id` → **`category`** on questionnaire concepts), optional **`prioritization_context`** (metadata only; not used to extract concepts)
  - **`synthesis_id`** is **content-addressed**: the same dossier + questionnaire payloads in the same run order yield the same id even when file paths differ (paths are not part of the hash).
  - Thresholds: top-level **`min_crops_for_pattern`** / **`min_mentions`** (defaults), or **`thresholds.default`** and **`thresholds.by_kind`** (e.g. stricter **`questionnaire_answer`** than dossier kinds). Set **`include_questionnaire_answer_blobs`** to `true` to emit `answer_markdown` concepts (off by default; claims-first via **`key_claims`**).
  - `--output-json`, `--render-markdown` — stdout prints full JSON (`render_synthesis_markdown` includes ontology edges and composite-rule primitives when present)
