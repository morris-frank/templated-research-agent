...
from contracts.agronomy.dossier import (
CropDossier,
LifecycleStage,
...
from contracts.core.questionnaire import (
QuestionAnswer,
QuestionnaireResponseSet,
QuestionnaireSpec,
)
from contracts.renderers.markdown import (
...
Claim(text="Wheat is often followed by barley or break crops depending on region", evidence_ids=["E002"])
],
...
status="answered",
answer_markdown=(
"- A reusable benchmark is a stage-specific pathogen pressure panel tied to emergence, canopy closure, and flowering.
"
...
(md_dir / "wheat.dossier.json").write_text(
    json.dumps(dossier.model_dump(mode="json"), indent=2), encoding="utf-8"
)
(md_dir / "wheat.questionnaire.json").write_text(
    json.dumps(response_set.model_dump(mode="json"), indent=2), encoding="utf-8"
)
...
print(f"Wrote demo artifacts to: {md_dir}")
